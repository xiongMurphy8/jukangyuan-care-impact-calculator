...
const policies = [
  ...
  ["所得税", "财税〔2009〕70号", "安置残疾人员就业企业所得税优惠政策", "https://fgk.chinatax.gov.cn/zcfgk/c102416/c5203518/content.html"],
  ["土地税", "财税〔2010〕121号", "安置残疾人就业单位城镇土地使用税政策", "https://fgk.chinatax.gov.cn/zcfgk/c102416/c5203544/content.html"],
];
const nationalBenefits = [
  ["增值税即征即退", "财税〔2016〕52号", "按安置残疾人人数限额即征即退；每人每月限额按当地月最低工资标准4倍确定。"],
  ["企业所得税加计扣除", "企业所得税法及财税〔2009〕70号", "残疾职工工资据实扣除基础上，再按支付给残疾职工工资的100%加计扣除。"],
  ["城镇土地使用税减免", "财税〔2010〕121号", "月平均实际安置残疾人比例达到25%且人数不少于10人，可按省级规定减征或免征。"],
  ["残保金分档减缴 / 免征", "财政部公告2023年第8号（执行至2027年12月31日）", "30人（含）以下企业继续免征；其他单位按安置比例适用分档减缴，地方征收口径需复核。"],
];
...
export default function Home() {
  ...
  const leadText = useMemo(() => [
    "聚康源爱心就业价值测算咨询",
    `称呼：${contactName || "未填写"}`,
    `联系方式：${contactMethod || "未填写"}`,
    `所在地区：${contactRegion || "未填写"}`,
    `近12个月内销业务应纳税额：${vat ? `¥ ${formatThousands(vat)} 元` : "未填写"}`,
    `上月社保申报在职职工人数：${people || "未填写"} 人`,
    `建议配置残疾职工：${c.ready ? `${c.hires} 人` : "待测算"}`,
    `预计年度净收益：${c.ready ? yuan.format(c.net) : "待测算"}`,
    `补充说明：${contactMessage || "无"}`,
  ].join("\n"), [contactName, contactMethod, contactRegion, vat, people, c.ready, c.hires, c.net, contactMessage]);
  ...
  return <main>
    <nav className="nav shell screen-only"><a className="brand" href="#top"><i>♥</i><span><b>聚康源</b><small>企业残疾人就业综合服务机构</small></span></a><div><a href="#calculator">开始测算</a><a href="#policies">政策依据</a><button onClick={makePdf}>生成 PDF 报告</button><button className="consult-nav" onClick={openLeadForm}>咨询复核</button></div></nav>
...
function LeadPrompt({ ready, net, hires, onOpen }: { ready: boolean; net: number; hires: number; onOpen: () => void }) {
  return <div className="lead-prompt"><div><small>NEXT STEP</small><h3>{ready ? "把初步结果变成正式方案":"先测算，再预约人工复核"}</h3><p>{ready ? `当前测算显示年度净收益约 ${yuan.format(net)}，建议配置 ${hires} 人。正式落地前，建议由聚康源顾问复核地区口径、业务性质和岗位安排。`:"填写两项数据后，可以生成 PDF 并预约聚康源顾问做一对一复核。"}</p></div><button type="button" onClick={onOpen}>获取正式复核方案</button></div>;
}
